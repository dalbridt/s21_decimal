...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
  ...
  if (!isinf(src) && !isnan(src) && dst != NULL) {
    *dst = (s21_decimal){0};
    if (src != 0) {
      uint32_t src_bits = *(uint32_t *)&src; // to get aka int but with the same bits order 
      ...
      if (exponent > -95 && exponent < 95) {
        ...
        // loop that makes 8 numbers before point (keeping track of scale) -  FIXME 
        // int k = scale + 7;
        // for (scale; scale < k; src *= 10, scale++) {
        // }  
        // printf("after loop 2 : %f || %d\n", src, scale);

        // get exponent after normalization 
        int exponent = ((src_bits & ~0x80000000) >> 23) - 127;
        set_bit(dst, exponent, 1); // тут по идее мы должны были установить 22 бит, но экспонента может быть  минусовая
        printf("hoba! %d\n", exponent); 
